...
<head>
  ...
  <style>
    ...
    #search-tower { margin-left:1em; padding:4px; width:150px; }
  </style>
</head>
<body>
  <nav class="navbar">
  ...
  <div id="controls">
    <form id="ranking-form" enctype="multipart/form-data" style="display:inline-block;">
    ...
    <div id="download-link" style="display:inline-block; margin-left:1em;"></div>
  </div>
  ...
  <script>
    ...
    // build an index of static towers
    const towerIndex = {};
      ...
      .then(data => {
        L.geoJSON(data, {
          pointToLayer: (f, latlng) => {
            ...
            const id = (f.properties.tower || f.properties.LEGEND || '').toString();
            if (id) {
              marker.bindPopup(`<strong>${id}</strong>`);
              // index it by uppercase ID
              towerIndex[id.toUpperCase()] = marker;
            }
            return marker;
          }
        }).addTo(map);
